...
def search_arxiv(query=None, author=None, year=None, max_results=1):
    ...
    if query is None and author is None and year is None:
        raise InputError("You must pass at least query, author or year.")
...
def make_powerpoint_slides(slides:str, filename:str="slides.pptx")->str:
    """
    ...
    Args:
        slides: A JSON string representing the slides. Each slide is expected to be a dictionary with
                a 'title' key and a 'content' key, where 'content' is a list of strings.
        filename: The name of the file to save the presentation to. Defaults to 'slides.pptx'.

    Returns: Filename of the PowerPoint file that was saved embedded in a Markdown sentence.
    """
    print("Creating PowerPoint slides", filename)
    import json
    ...
    try:
        from IPython.display import display, Markdown
        display(Markdown(f"The Powerpoint Presentation was saved as [{filename}]({filename})"))
    except:    
        pass
...
def create_agent(name, description, tools, model="meta-llama/Llama-3.3-70B-Instruct", system_message=None, verbose=False, max_iterations=20):
    ...
    if system_message is not None:
        chat_history = [ChatMessage(system_message, role=MessageRole.SYSTEM)]

    agent = ReActAgent.from_tools(
        [FunctionTool.from_defaults(fn=t) for t in tools], 
...
def prompt_openai(message:str, model="gpt-4o"):
